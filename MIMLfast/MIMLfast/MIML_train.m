function [W,V,AW,AV,Anum,trounds]=MIML_train(train_data,train_targets,W,V,costs,norm_up,step_size0,num_sub,AW,AV,Anum,trounds,lambda,opts,beta)
...
%         idx_ins
        tmp1=Wy+step_size*beta(idx_ins)*loss*Vbag(:,idx_max_ins1);%V*xins1;
        if(opts.norm~=0)
        tmp3=norm(tmp1);
        if(tmp3>norm_up)
